...
void printOSStatus(char *name, OSStatus status) {
    if(debuglog) {
        ...
        NSLog(@"%s:%@", name, [error description]);
    }
}
...
void rtAudioInitWithCallback(OSStatus(*renderCallback)(void *userData,
    ...
    success = [[AVAudioSession sharedInstance] setCategory:AVAudioSessionCategoryAmbient error:&error];
    if(!success) {
        if(debuglog) {
            NSLog(@"AVAudioSessionCategoryAmbient %@", error);
        }
        return;
    }
